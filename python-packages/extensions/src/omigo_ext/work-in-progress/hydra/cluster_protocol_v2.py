...
class ClusterHeartbeatProtocol:
    ...
    def update_heartbeat(self):
        while True:
            if (self.update_heartbeat_inner() == True):
                ...
                # TODO
                utils.debug("ClusterHeartbeatProtocol: update_heartbeat exiting")
                return
    ...
    def __is_alive__(self):
        ...
        if (self.cluster_handler.dir_exists(ClusterPaths.get_entity_heartbeat(self.get_entity_type(), self.get_entity_id())) == False):
            utils.debug("ClusterHeartbeatProtocol: __is_alive__: entity missing: {}".format(self.get_entity_id()))
            return False
        ...
        if (heartbeat_server_timestamp is None):
            utils.debug("ClusterHeartbeatProtocol: __is_alive__: entity details missing: {}".format(self.get_entity_id()))
            return False
        ...
        if (time_diff > ClusterHeartbeatProtocol.MAX_HEARTBEAT_WAIT):
            utils.debug("ClusterHeartbeatProtocol: __is_alive__: {}, heartbeat too old. Time Diff: {} seconds. Exiting".format(self.get_entity_id(), time_diff))
            return False
        else:
            return True
    ...
    def update_heartbeat_inner(self):
        ...
        if (self.__is_alive__() == False):
            utils.debug("ClusterHeartbeatProtocol: update_heartbeat: {}, heartbeat missing or too old. Exiting".format(self.get_entity_id()))
            return False
...
class ClusterEntityStateProtocol:
    ...
    def get_registered_state_resolved(self):
        ...
        # invalid
        raise Exception("ClusterEntityStateProtocol: get_registered_state_resolved: invalid: {}".format(self.entity_id))
...
class ClusterEntityProtocol:
    ...
    def initialize(self):
        ...
        # create dependents 
        for xchild_type in cluster_common_v2.EntityDepdendentsMap[self.get_entity_type()]:
            ...
            if (xchild_entity.entity_type == EntityType.WF):
                ...
                if (xchild_entity.entity_spec.is_live == True):
                    ...
                    if (len(available_agent_ids) > 0):
                        ...
                        selected_agent_id = alive_agent_ids[0]
    ...
    def do_cleanup(self, xchild_entity):
        ...
        class_reflectionentity_cleanup_protocol = ClusterEntityCleanupProtocol(xchild_entity.entity_type, xchild_entity.entity_id)
    ...
    def do_execute_passive_child(self, xchild_entity):
        # check the entity_type for child
        xchild_entity_type = xchild_entity.entity_type

        # switch case for different child entity_types
        if (xchild_entity_type == EntityType.SWF):
            pass
        elif (xchild_entity_type == EntityType.WF):
            pass
        elif (xchild_entity_type == EntityType.JOB):
            pass
        elif (xchild_entity_type == EntityType.TASK):
            pass
        elif (xchild_entity_type == EntityType.BATCH):
            pass
        else:
            raise Exception("ClusterEntityProtocol: {}: do_execute_passive_child: {}: unknown xchild_entity_type: {}".format(self.get_entity_id(), xchild_entity.entity_id, xchild_entity_type))
            ...
            for xchild_entity_id in xchild_entity_ids:
                ...
                if (cur_registered_state == EntityState.CREATED):
                    ...
                    if (xchild_entity_type == EntityType.WF):
                        self.do_execute_execution_task_wf(xchild_entity)
                elif (cur_registered_state == EntityState.COMPLETED):
                    pass
                elif (cur_registered_state == EntityState.FAILED):
                    pass
    ...
    def do_execute_execution_task_wf(self, xchild_entity):
        ...
        # check if it is live
        if (entity_spec.is_live == True):
            self.do_execute_execution_task_wf_live(xchild_entity)
        else:
            raise Exception("do_execute_execution_task_wf: not implemented yet")

    def do_execute_execution_task_wf_live(self, wf_entity):
        # instantiate protocol
        wf_protocol = ClusterWFProtocol(wf_entity)
        utils.info("ClusterEntityProtocol: {} do_execute_execution_task_wf_live: executing workflow: {}".format(self.get_entity_id(), wf_entity.entity_id))
        wf_protocol.execute()
...
class ClusterMasterProtocol(ClusterEntityProtocol):
    ...
    def __select_assigned_supervisor_entity__(self, xentity_type):
        ...
        if (xsupervisor_entity_type == EntityType.MASTER):
            ...
            xsupervisor_ids = self.cluster_handler.list_dirs(ClusterPaths.get_entities(xsupervisor_entity_type))
            ...
            if (len(xalive_entity_ids) > 0):
                ...
                random.shuffle(xalive_entity_ids)
...
class ClusterWFProtocol(ClusterEntityProtocol):
    ...
    def execute_static(self):
        raise Exception("ClusterWFProtocol: {} execute: non live workflow not implemented yet".format(self.get_entity_id()))
    ...
    def __resolve_reference_paths__(self, xtsv, wf_start_ts, use_full_data, start_ts, end_ts):
        ...
        for c in xtsv.get_columns():
            if (c.startswith(cluster.OMIGO_ETL_PATH_PREFIX)):
        ...
        if (etl_path_col is not None):
            ...
            for mp in mps:
                ...
                etl_start_ts = mp[cluster_temp.OMIGO_AAAAA_ETL_START_TS]
                etl_end_ts = mp[cluster_temp.OMIGO_AAAAA_ETL_END_TS]

                # TODO: HACK. Synchronizing dependencies
                etl_input_paths = etl.get_file_paths_by_datetime_range(etl_full_path, etl_start_ts, etl_end_ts, cluster_temp.OMIGO_AAAAA_ETL_FILE_PREFIX)
                ...
                etsv = etl.scan_by_datetime_range(etl_full_path, effective_etl_start_ts, etl_end_ts, cluster_temp.OMIGO_AAAAA_ETL_FILE_PREFIX)
                # etsv.show_transpose(4)
    ...
    def __resolve_meta_params__(self, xtsv, start_ts, end_ts):
        def __resolve_meta_params_inner__(x):
            ...
            cols = {
                cluster_temp.OMIGO_AAAAA_START_TS: start_ts_str,
                cluster_temp.OMIGO_AAAAA_END_TS: end_ts_str
            }

            # resolve
            for k in cols.keys():
                c = "{" + k + "}"
                x = x.replace(c, cols[k])
    ...
    def execute_live(self):
        ...
        operations = self.__execute_live_single_round_get_operations__(wf_spec)
        ...
        for iter_count in range(num_iter):
            ...
            # call a single run
            xoutput = self.__execute_live_single_round__(operations, xinput_resolved)
    ...
    def __resolve_etl_file_timestamp__(self, xtsv, start_ts, end_ts):
        ...
        if (xtsv.has_col(cluster_temp.OMIGO_AAAAA_EVENT_TS)):
            # take the min max values
            col_values = sorted(xtsv.col_as_array_uniq(cluster_temp.OMIGO_AAAAA_EVENT_TS))
    ...
    def __execute_live_single_round__(self, operations, xinput_resolved):
        ...
        for (operation, extend_class_op) in operations:
            ...
            try:
                ...
                utils.debug("ClusterWFProtocol: {} __execute_live_single_round__: extend_class_op: {}, extend_class_obj: {}".format(self.get_entity_id(), extend_class_op, extend_class_obj))
                ...
                # debug
                utils.debug("ClusterWFProtocol: {} __execute_live_single_round__: custom_func: name: {}, args: {}, kwargs: {}, extend_class_obj: {}, func: {}".format(self.get_entity_id(), operation.name, args, kwargs, extend_class_obj, class_func))

                # note that custom_func is not called as the context is already in the function.
                otsv = class_func(*args, **kwargs)
            except Exception as e:
                utils.error("ClusterWFProtocol: {} __execute_live_single_round__: Found error while running the method: {}".format(e))
                raise e

        # return output
        return otsv

    def __execute_live_single_round_get_operations__(self, wf_spec):
        # create tasks
        operations = []
...
class ClusterExecutorContext:
    def __init__(self, session_protocol, is_live = False, interval = 60, duration = 300, output_id = "output0"):
        self.cluster_handler = ClusterPaths.get_cluster_handler()
    ...
    def execute_jobs(self, xtsv, jobs_operations, output_ids = None, start_ts = None, use_full_data = False, num_splits = 10):
        # create workflow semantics from job
        wf_spec = self.__create_wf_spec__(jobs_operations, output_ids, start_ts, use_full_data, num_splits)
    ...
    # create wf spec
    def __create_wf_spec__(self, jobs_operations, output_ids, start_ts, use_full_data, num_splits):
        # jobs_specs
        jobs_specs = []
        ...
        # create workflow
        wf_spec = cluster_common_v2.ClusterSpecWF.new(jobs_specs, is_live = self.is_live, interval = self.interval, start_ts = start_ts, use_full_data = use_full_data, duration = self.duration, output_ids = output_ids)
