...
implementations: `28-phone-motion-sensors` (one trace, two readings),
`29-eye-tracking` (one error, two consequences), `25-biometric-templates-matching`
...
- **Verify charts by executing them.** `node --check` parses but does not run; an
  out-of-scope variable inside a canvas IIFE passes it and throws at runtime, leaving
  a blank canvas that looks like a CSS bug. The harness must stub a canvas context,
  extract each `<script>`, actually execute it, and exit nonzero on a throw.
- **A check that cannot fail is not a check.** Confirm it reports failure on known-bad
  input first. A whole batch was once "verified" by a harness that read nothing.
