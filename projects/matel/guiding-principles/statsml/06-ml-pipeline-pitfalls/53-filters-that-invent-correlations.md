...
|---|----------|-------|------|-------------|--------|
| 1 | INTRO | The Door Makes the Trade-Off | [53-filters-that-invent-correlations/01-the-door-makes-the-trade-off.md](53-filters-that-invent-correlations/01-the-door-makes-the-trade-off.md) | Start here: why "good at either one" turns two unrelated skills into rivals, using pro basketball rosters. | selection-bias, collider, intuition |
| 2 | QUERY | Filters and Joins That Cut a Corner | [53-filters-that-invent-correlations/02-filters-and-joins-that-cut-a-corner.md](53-filters-that-invent-correlations/02-filters-and-joins-that-cut-a-corner.md) | A `WHERE` clause, an inner join, and a drop-nulls step are all entry rules — and each bends the correlation matrix. | WHERE-clause, inner-join, drop-nulls |
| 3 | LABELS | Training Only on Who Got Approved | [53-filters-that-invent-correlations/03-training-only-on-who-got-approved.md](53-filters-that-invent-correlations/03-training-only-on-who-got-approved.md) | Labels exist only for applicants a gate let through, so the model learns the gate's shape, not the world's. | approved-only, reject-inference, labels |
| 4 | FEEDBACK | The Model Filters Its Own Next Dataset | [53-filters-that-invent-correlations/04-the-model-filters-its-own-next-dataset.md](53-filters-that-invent-correlations/04-the-model-filters-its-own-next-dataset.md) | Each retrain sees only rows the last version let through, and the feature it trusted most goes flat. | feedback-loop, retraining, exploration |
